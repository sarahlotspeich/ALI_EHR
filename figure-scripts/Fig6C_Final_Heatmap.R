...
with(data, sum(UNVAL == "Yes" & VAL == "Yes") / sum(VAL == "Yes" & UNVAL != "Missing")) #### 99.5%

## Calculate true positive rate (FPR) = P(UNVAL = 1 | VAL = 0)
with(data, sum(UNVAL == "Yes" & VAL == "No") / sum(VAL == "No" & UNVAL != "Missing")) #### 2%
...
ggsave(filename = "~/Documents/ALI_EHR/figures/Fig6C_Final_Heatmap.png",

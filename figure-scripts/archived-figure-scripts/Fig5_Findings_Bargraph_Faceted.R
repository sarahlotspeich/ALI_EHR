...
long_audit_dat |> 
  ...
  mutate(
    ...
    Finding = factor(x = Finding, 
                     ...
                     labels = rev(c("Extracted Value Correct", 
                                    "Extracted Value Incorrect", 
                                    "Extracted Value Not Found", 
                                    "Extracted Value Missing and No Auxiliary Information Found",
                                    "Extracted Value Missing and Auxiliary Information Found" 
                     ))), 
    Missing = case_when(grepl(pattern = "Auxiliary", x = Finding) ~ "Missing in Extracted Data", 
                        .default = "Non-Missing in Extracted Data"),
    Missing = factor(x = Missing, 
                     levels = c("Non-Missing in Extracted Data", 
  ...
  coord_flip() + 
  facet_grid(rows = vars(Missing), 
             cols = vars(Audit))

## Save it
ggsave(filename = "~/Documents/ALI_EHR/figures/Fig5_Findings_Bargraph_Faceted.png", 

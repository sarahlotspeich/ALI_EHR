...
nrow(long_audit_dat1) ## 7605 audited data points 
table(long_audit_dat1$Category) ## 1049 labs, 6556 vitals
...
## Pilot + Wave I audits 
summ_audit_dat1 = long_audit_dat1 |> 
...
long_audit_dat1 |> 
  mutate(Finding = factor(x = Finding, 
                          levels = c("Extracted Value Correct", 
                                     "Extracted Value Incorrect", 
                                     "Extracted Value Not Found", 
                                     "No Auxiliary Information Found",
...
## Wave II audits 
summ_audit_dat2 = long_audit_dat2 |> 
  group_by(Category, Variable_Name, Finding) |> 
  summarize(Num = n()) |> 
  group_by(Category, Variable_Name) |> 
  mutate(Total_Num = sum(Num), 

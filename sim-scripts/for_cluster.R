...
source("logreg2phRonly/R/logreg2ph.R")
source("logreg2phRonly/R/hessian_row.R")
source("logreg2phRonly/R/observed_data_loglik.R")
source("logreg2phRonly/R/pl_theta.R")
source("logreg2phRonly/R/profile_out.R")
...
tpr99_fpr01 = do.call(what = rbind,
  ...
  dplyr::mutate(tpr = 0.99, 
                fpr = 0.01)
tpr99_fpr01 |> 
  write.csv(paste0("vary_designs_tpr99_fpr01_seed", sim_seed, ".csv"), 
            row.names = FALSE)
...
tpr95_fpr05 = do.call(what = rbind,
  ...
  dplyr::mutate(tpr = 0.95, 
                fpr = 0.05)
tpr95_fpr05 |> 
  write.csv(paste0("vary_designs_tpr95_fpr05_seed", sim_seed, ".csv"), 
            row.names = FALSE)
...
tpr80_fpr20 = do.call(what = rbind,
  ...
  dplyr::mutate(tpr = 0.8, 
                fpr = 0.2)
tpr80_fpr20 |> 
  write.csv(paste0("vary_designs_tpr80_fpr20_seed", sim_seed, ".csv"), 
            row.names = FALSE)
...
tpr50_fpr50 = do.call(what = rbind,
  ...
  dplyr::mutate(tpr = 0.5, 
                fpr = 0.5)
tpr50_fpr50 |> 
  write.csv(paste0("vary_designs_tpr50_fpr50_seed", sim_seed, ".csv"), 
            row.names = FALSE)

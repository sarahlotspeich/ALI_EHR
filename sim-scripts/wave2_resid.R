...
sim_data_fit = function(id, tpr, fpr) {
  ...
  results[1, paste0("resid_beta", 0:2)] = fit$coefficients$Estimate
  results[1, "resid_conv_msg"] = fit$converge_msg
...
tpr99_fpr01 = do.call(what = rbind,
  ...
  dplyr::mutate(tpr = 0.99, 
                fpr = 0.01)
tpr99_fpr01 %>% 
  write.csv(file = "vary_wave2_design/wave2_residual_tpr99_fpr01.csv", 
            row.names = FALSE)
...
tpr95_fpr05 = do.call(what = rbind,
  ...
  dplyr::mutate(tpr = 0.95, 
                fpr = 0.05)
tpr95_fpr05 %>% 
  write.csv(file = "vary_wave2_design/wave2_residual_tpr95_fpr05_seed.csv", 
            row.names = FALSE)
...
tpr80_fpr20 = do.call(what = rbind,
  ...
  dplyr::mutate(tpr = 0.8, 
                fpr = 0.2)
tpr80_fpr20 %>% 
  write.csv(file = "vary_wave2_design/wave2_residual_tpr80_fpr20_seed.csv", 
            row.names = FALSE)
...
tpr50_fpr50 = do.call(what = rbind,
  ...
  dplyr::mutate(tpr = 0.5, 
                fpr = 0.5)
tpr50_fpr50 %>% 
  write.csv(file = "vary_wave2_design/wave2_residual_tpr50_fpr50_seed.csv", 
            row.names = FALSE)
